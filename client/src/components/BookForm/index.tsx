...
dayjs.extend(utc)
 ...
 const BookForm = () => {
    ...
    const formSubmit = (event: any) => {
        ...
        if (!isInvalid) {
            if (bookIsbn) {
                const newPrice = parseInt(String(book.price), 10)
                const newQuantity = parseInt(String(book.quantity), 10)
    ...
    const updateBookField = (event: any) => {
    ...
    return (
        <>
            <Container component={Paper} className={"wrapper"}>
                <Typography className={"pageHeader"} variant="h5">
